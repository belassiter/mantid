...
// Win conditions
const WIN_CONDITION_STANDARD = 10;
const WIN_CONDITION_TWO_PLAYER = 15;

// Check if a player has won
const checkWinCondition = (scoreCount, numPlayers) => {
  const target = numPlayers === 2 ? WIN_CONDITION_TWO_PLAYER : WIN_CONDITION_STANDARD;
  return scoreCount >= target;
};

// Check if it's a specific player's turn
const isPlayerTurn = (currentPlayerIndex, playerIndex) => {
  return currentPlayerIndex === playerIndex;
};

// Get next player index
const getNextPlayerIndex = (currentPlayerIndex, numPlayers) => {
  return (currentPlayerIndex + 1) % numPlayers;
};

// Validate if a player can perform an action
const canPerformAction = (game, playerId) => {
...
const determineTiebreaker = (players) => {
...
module.exports = {
  WIN_CONDITION_STANDARD,
  WIN_CONDITION_TWO_PLAYER,
  checkWinCondition,
  isPlayerTurn,
  getNextPlayerIndex,
  canPerformAction,
